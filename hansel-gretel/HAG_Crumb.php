...
final class HAG_Crumb {
	...
	private function get_id() {
		$id = '';
		if ( $this->is_home ) {
			$id = $this->options['home_id'];
		}
		if ( $this->is_last ) {
			$id = $this->options['last_id'];
		}
	...
	private function get_class() {
		...
		if ( $this->is_home ) {
			$class[] = $this->options['home_class'];
		}
		if ( $this->is_last ) {
			$class[] = $this->options['last_class'];
		}
	...
	private static function get_home_crumbs( array $options ) {
		...
		if ( ! $bh && ! is_null( $post ) && 'post' !== $post->post_type ) {
			return $crumbs;
		}
		if ( is_search() || is_404() ) {
			return $crumbs;
	...
	private static function get_author_crumbs( array $options ) {
		...
		if ( is_null( $author ) || is_wp_error( $author ) ) {
			return $crumbs;
		}
	...
	private static function get_post_type_crumbs( array $options ) {
		$crumbs = array();
		if ( ! $options['last_show'] || ! $options['post_type_show'] ) {
			return $crumbs;
		}
	...
	private static function get_taxonomy_crumbs( array $options ) {
		...
		if ( is_null( $tax ) || is_wp_error( $tax ) ) {
			return $crumbs;
		}
